...
export default function VehicleModal({ vehicle, isOpen, onClose, reservationInfo }: VehicleModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      ...
      <DialogContent className="w-[95vw] sm:max-w-[500px] md:max-w-[600px] max-h-[90vh] sm:max-h-[85vh] p-0 overflow-hidden bg-black/70 backdrop-blur-md text-white border border-[#8e7d3f]/30 shadow-lg">
        <div className="max-h-[calc(90vh-80px)] sm:max-h-[calc(85vh-120px)] overflow-y-auto px-4 sm:px-6 py-6">
          {step === "details" && (
            <div className="relative">
              {/* En-tête du modal */}
              <div className="mb-6">
                <h2 className="text-xl sm:text-2xl font-bold mb-6">{vehicle.name}</h2>
                <p className="text-sm text-gray-400">{vehicle.type}</p>
                <p className="text-xs text-gray-300">{vehicle.subtitle}</p>
              </div>

              {/* Image du véhicule */}
              <div className="relative mb-6 h-48 sm:h-64 bg-gradient-to-b from-zinc-800/50 to-zinc-900/50 rounded-lg overflow-hidden border border-[#8e7d3f]/20">
                <img
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-t pt-6">
                <div>
                  {totalPrice ? (
                    <div>
                      <p className="text-xl sm:text-2xl font-bold">{totalPrice.toFixed(2)} CHF</p>
                      <p className="text-xs sm:text-sm text-gray-400">
                        {reservationInfo?.withDriver ? (
                          <span>
                            {(vehicle.pricePerKm + (vehicle.withDriverSupplement || 0)).toFixed(2)} CHF/km avec
                            chauffeur
                          </span>
                        ) : (
                    ...
                    <p className="text-xl sm:text-2xl font-bold">{vehicle.pricePerKm.toFixed(2)} CHF /km</p>
                  )}
                </div>
                <Button
                  className="w-full sm:w-auto rounded-lg bg-[#8e7d3f] px-4 sm:px-6 py-2 text-white hover:bg-[#8e7d3f]/90 transition-colors"
          ...
          {step === "confirmation" && (
            <div className="relative text-center py-8">
              ...
              <p className="text-gray-300 mb-6">
                Merci pour votre réservation. Un email de confirmation a été envoyé à {formData.email}. Notre équipe
                vous contactera prochainement pour finaliser les détails.
              </p>
